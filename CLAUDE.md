...
- **Orders are legacy CPT storage (wp_posts), NOT HPOS.** Order queries hit `wp_posts` / `wc_order_product_lookup`. Membership grants fire on order status → processing/completed only, and only for orders linked to a WP account (guests grant nothing). If WooCommerce order storage is ever migrated to HPOS, the order re-scan, kit-purchase detection, and membership seeding queries MUST be ported to `wc_get_orders()` first — they will silently return nothing otherwise.
