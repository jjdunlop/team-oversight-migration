...
- **After saving**: when no payment is needed, submitting or saving changes reloads the rego page itself — green status panel with the trial number, the form ready for more edits — with a one-off notice on top ("Thanks — your application has been submitted" / "Your changes have been saved"). When the fee is due it goes straight to checkout instead.
